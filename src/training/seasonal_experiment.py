...
Run: python -m src.training.seasonal_experiment [model-config-path]
"""
...
def build_seasonal_dataloaders(
    ...
    train_ds = ShardDataset(warm_train, shards_dir=shards_dir, split=None)
    val_ds = ShardDataset(warm_val, shards_dir=shards_dir, split=None)
    test_ds = ShardDataset(cool, shards_dir=shards_dir, split=None)
...
def main(max_epochs: int | None = None, model_config_path: str = DEFAULT_MODEL_CONFIG) -> None:
    ...
    with mlflow.start_run(run_name=f"{architecture}_seasonal_experiment"):
        mlflow.log_params(
            {
                "architecture": architecture,
    ...
    checkpoint_path = Path(f"models/checkpoint_{architecture}_seasonal.pt")
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(module.model.state_dict(), checkpoint_path)
    print(f"wrote {checkpoint_path}")
    print(
        f"for the full predictions CSV + plots, run: "
        f"python -m src.training.evaluate {architecture} seasonal"
    )
